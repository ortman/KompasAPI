#if !defined(_KSAPI_KOMPAS_HPP_) && defined(KSAPI)
#define _KSAPI_KOMPAS_HPP_
//#include "Kompas3D.h"
//#include "Doc3D.h"
...
private:
	ksapi::IApplication* kompasApp;

public:
	K3D_KS_Kompas3D(ksapi::IApplication& app, ksapi::IKompasLibraryActions& libaryActions) : kompasApp(&app) {
...
//					return Doc3D(K3D_KS_Doc3D(doc3d));
				}
			}
		}
		return Doc3D();
	...
	std::wstring SystemPath(long type) override {
		if (kompasApp) {
			...
			if (settings) {
				...
				//return L"C:\\Users\\User\\AppData\\Roaming\\ASCON\\KOMPAS-3D\\24";
			}
		}
		return L"";
	}
...
//	Kompas3D::SetKompas(std::move(std::make_unique<K3D_KS_Kompas3D>(app, libaryActions)));
//	return true;
//}
//
//APP_EXP_FUNC(void) UnloadKompasLibrary() {
//	Kompas3D::SetKompas(std::make_unique<K3D_Kompas3D>());
//}
#endif